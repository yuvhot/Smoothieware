...
bool MotorDriverControl::config_module(uint16_t cs) {
    std::string str = THEKERNEL->config->value(motor_driver_control_checksum, cs, axis_checksum)->by_default(
            "")->as_string();
    if (str.empty()) {
        // NOTE Deprecated use of designator for backward compatibility
        str = THEKERNEL->config->value(motor_driver_control_checksum, cs, designator_checksum)->by_default(
                "")->as_string();
        if (str.empty()) {
    ...
    if (DRV->connection_method == StepstickParameters::UART) {
        ...
        sw_uart_tx_pin->from_string(
                THEKERNEL->config->value(motor_driver_control_checksum, cs, sw_uart_tx_pin_checksum)->by_default(
                        "nc")->as_string())->as_output();
        ...
        if (THEKERNEL->config->value(motor_driver_control_checksum, cs, sw_uart_rx_pin_checksum)->by_default(
                "nc")->as_string() != "nc") {
            sw_uart_rx_pin->from_string(
                    THEKERNEL->config->value(motor_driver_control_checksum, cs, sw_uart_rx_pin_checksum)->by_default(
                            "nc")->as_string())->as_input();
        ...
        int sw_uart_baudrate = THEKERNEL->config->value(motor_driver_control_checksum, cs,
                                                        sw_uart_baudrate_checksum)->by_default(9600)->as_number();
        ...
        spi_cs_pin = new Pin();
        spi_cs_pin->from_string(
                THEKERNEL->config->value(motor_driver_control_checksum, cs, spi_cs_pin_checksum)->by_default(
                        "nc")->as_string())->as_output();
    ...
    max_current = THEKERNEL->config->value(motor_driver_control_checksum, cs, max_current_checksum)->by_default(
            (int) DRV->max_current)->as_number(); // in mA
    //current_factor= THEKERNEL->config->value(motor_driver_control_checksum, cs, current_factor_checksum )->by_default(1.0F)->as_number();

    current = THEKERNEL->config->value(motor_driver_control_checksum, cs, current_checksum)->by_default(
            1000)->as_number(); // in mA
    microsteps = THEKERNEL->config->value(motor_driver_control_checksum, cs, microsteps_checksum)->by_default(
            16)->as_number(); // 1/n
    ...
    str = THEKERNEL->config->value(motor_driver_control_checksum, cs, raw_register_checksum)->by_default(
            "")->as_string();
    if (!str.empty()) {
        rawreg = true;
    ...
    if (!write_only &&
        THEKERNEL->config->value(motor_driver_control_checksum, cs, alarm_checksum)->by_default(false)->as_bool()) {
        halt_on_alarm = THEKERNEL->config->value(motor_driver_control_checksum, cs, halt_on_alarm_checksum)->by_default(
                false)->as_bool();
    ...
    if (DRV->connection_method == StepstickParameters::UART) {
        printf("MotorDriverControl INFO: configured motor %c (%d): as %s, tx: %04X, rx: %04X\n",
               axis,
               id,
               THEKERNEL->config->value(motor_driver_control_checksum, cs, chip_checksum)->by_default(
                       "")->as_string().c_str(),
            ...
            if (gcode->has_letter(axis)) {
                ...
                if (gcode->subcode == 1 && current_microsteps != microsteps) {
                    ...
                    if (a < THEROBOT->get_number_registered_motors()) {
                        float s =
                                THEROBOT->actuators[a]->get_steps_per_mm() * ((float) microsteps / current_microsteps);
...
void MotorDriverControl::on_homing(void *argument) {
    EventState *event_state = (EventState *) argument;
    if (event_state != nullptr) {
        if (*event_state == EventState::PRE) {
            DRV->pre_homing();
        } else if (*event_state == EventState::POST) {
            DRV->post_homing();
        }
    }
}